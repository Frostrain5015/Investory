...
public class StrategyDao extends BaseDao {
    ...
    /**
     * 应用启动后自动执行：确保 {@code backtest_strategies} 表存在 {@code config_json} 列。
     *
     * <p>该方法使用 {@code ALTER TABLE ADD COLUMN} 进行幂等式表结构迁移。
     * 若列已存在，MySQL 会抛出异常，此处通过 {@code catch (Exception ignored)} 静默忽略，
     * 确保启动过程不因重复执行而中断。</p>
     *
     * <p>这是一种轻量级的"就地迁移"方式，避免引入完整的数据库迁移框架（如 Flyway）。</p>
     */
    @PostConstruct
    private void ensureConfigColumn() {
        try {
            // 若列不存在则添加；若已存在则抛出异常被静默忽略
