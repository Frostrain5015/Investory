...
public class AuthService {
    ...
    @Autowired private UserDao userDao;           // 用户数据访问对象
    @Autowired private PortfolioDao portfolioDao; // 投资组合数据访问对象

    /**
     * 注册新用户，并自动为其创建一个默认投资组合。
     *
     * <p>整个操作在事务中执行（{@code @Transactional(rollbackFor = Exception.class)}），
     * 若插入用户或创建投资组合任一失败，均会完整回滚。
     *
     * <p>校验规则（按顺序）：
     * <ol>
     *   <li>用户名不能为空</li>
    ...
    public String register(String username, String password, String email) {
        ...
        user.setPasswordHash(BCrypt.hashpw(password, BCrypt.gensalt())); // BCrypt 自动生成随机盐值
